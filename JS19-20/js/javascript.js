$(function(){
	var arr = [
		...
		var unparsed = JSON.parse(parsed);
		var count = 0;
		var Skills = [];
		for (var i = 0; i<unparsed.length; i++){
			for (var j=0; j<unparsed[i].skills.length; j++){
				Skills[count] = unparsed[i].skills[j];
				count++;
			};
		};
		var compactSkills = _.uniq(Skills);
		var sortSkills=_.sortBy(compactSkills);
		console.log("Массив скиллов всех людей: " + sortSkills);
		var sortName = _.sortBy(arr, 'friends.length');
		var names = [];
		for (var i = 0; i<sortName.length; i++){
			names[i] = sortName[i].name;
		};
		console.log('Массив имен: ' + names);
		var count = 0;
		var Friends = [];
		for (var i = 0; i<unparsed.length; i++){
			for (var j=0; j<unparsed[i].friends.length; j++){
				Friends[count] = unparsed[i].friends[j].name;
				count++;
			};
		};
		var compactFriends = _.uniq(Friends);
		console.log("Массив друзей: " + compactFriends);
});